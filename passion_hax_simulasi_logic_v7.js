...
// Realism Variable 1: Tier Difficulty Multiplier
function getDifficultyMultiplier(income) {
  if (income < 20000000) return 1.0; // Tier 1/2: Starter / Executor
  if (income < 40000000) return 0.8; // Tier 3: Specialist
  if (income < 80000000) return 0.6; // Tier 4: Strategist
...
function getAgeMultiplier(currentAge, income) {
  if (currentAge >= 40 && income < 40000000) return 0.75; // 25% penalty
  if (currentAge >= 35 && income < 20000000) return 0.85; // 15% penalty
  return 1.0;
}
...
function calculateProjection() {
  ...
  let initialEquity = userData.income >= 80000000 ? userData.income + (userData.income - 80000000) * 1.5 : userData.income;
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    if (dataC[i-1] >= 80000000) currentIntervalC = Math.max(chartParams.interval, 5);
    else if (dataC[i-1] >= 40000000) currentIntervalC = Math.max(chartParams.interval, 4);
    
    if (dataB[i-1] >= 80000000) currentIntervalB = Math.max(chartParams.interval, 5);
    else if (dataB[i-1] >= 40000000) currentIntervalB = Math.max(chartParams.interval, 4);
    ...
    if (dataC[i] >= 80000000) {
      let equityNominal = (dataC[i] - 80000000) * 1.5;
...
function updateNarrative() {
  ...
  // Show equity alert when Path C reaches 80M
  if (maxC >= 80000000) {
  ...
  if (maxA < 80000000 && maxA >= 40000000) { 
